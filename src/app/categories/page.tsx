          ...
          >
            <Link href={`/categories/${category.id}`} className="block h-full">
              ...
              >
                ...
                <div className={`absolute top-0 right-0 w-32 h-32 ${category.color} blur-[80px] opacity-5 dark:opacity-10 group-hover:opacity-10 dark:group-hover:opacity-20 transition-opacity`} />

                <div className="relative z-10">
                  {/* Icon Container */}
                  <div className={`w-14 h-14 rounded-xl ${category.color}/10 dark:${category.color}/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}>
                    {/* Icon Color: Dynamic color in Light Mode, White in Dark Mode */}
                    <category.icon size={28} className={`text-gray-900 dark:text-white`} />
                  </div>
